<x-mail::message>
# {{ __('mail.orders.admin_placed_heading', ['id' => $order->id]) }}

{{ __('mail.orders.admin_placed_intro') }}
...
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="font-size: 13px; margin: 28px 0; border-collapse: collapse;">
    <tr>
        <td width="50%" valign="top" style="padding-right: 12px;">
            <strong>{{ __('mail.orders.customer') }}</strong><br>
            {{ $order->customer_name }}<br>
            {{ __('mail.orders.phone') }}: {{ $order->customer_phone }}<br><br>
            <strong>{{ __('mail.orders.shipping_address') }}</strong><br>
            {{ $order->shipping_address }}<br>
            {{ $order->city }}{{ $order->state ? ', '.$order->state : '' }}<br>
            {{ $order->zip }}, {{ $order->country }}
        </td>
        <td width="50%" valign="top" style="padding-left: 12px;">
            <strong>{{ __('mail.orders.delivery_method') }}</strong><br>
            {{ $order->delivery_method_name }}<br>
            {{ number_format($order->delivery_price, 2) }} {{ $order->currency }}<br><br>
            <strong>{{ __('mail.orders.payment_method') }}</strong><br>
            {{ $order->payment_method_name }}<br>
            {{ __('mail.orders.payment_status', ['status' => $order->payment_status]) }}
        </td>
    </tr>
</table>
